			...
			public void afterInit(MinecraftClient client, Screen screen, int scaledWidth, int scaledHeight) {
				...
				if (config.isServerForced() && config.isEnabled()) {
					if (screen instanceof TitleScreen) {
						ServerInfo info = new ServerInfo("Server", config.getServerIP(), false);
						ConnectScreen.connect(new MultiplayerScreen(null), MinecraftClient.getInstance(), ServerAddress.parse(config.getServerIP()), info, false);
					}
				}
			}
		});
